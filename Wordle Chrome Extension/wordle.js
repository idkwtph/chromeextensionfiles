...
function storageChanged(changes, area) {
  ...
  for (const item of changedItems) {
    if (item == "darkMode") darkMode = changes[item].newValue;
    if (item == "autoFill") {
      autoFill = changes[item].newValue;
    }
    if (item == "wordleSync") {
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.greeting === "hello") {
    if (location.href != "https://www.nytimes.com/games/wordle/index.html") {
      sendResponse(`Error`);
    } else {
      sendResponse(`${answer}`);
    }
  ...
  } else if (request.greeting === "darkMode") {
    darkMode = request.message;
  } else if (request.greeting === "autoFill") {
    autoFill = request.message;
  }
  return false;
});

let firstChar = answer.charAt(0);
let secondChar = answer.charAt(1);
let thirdChar = answer.charAt(2);
